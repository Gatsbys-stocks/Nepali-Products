...
- **Catálogo gestionable por el propio cliente**: la página pide el catálogo al backend (`GET /api/products`) en vez de tenerlo escrito en el código, así que cuando el dueño del negocio añade, cambia o marca como agotado un producto desde su panel de administración, aparece en la web sin que yo tenga que tocar nada. Si el backend no responde, la página se apoya en un catálogo de respaldo para no quedarse en blanco.
- **Control de stock**: un producto marcado como agotado se muestra atenuado con un aviso "Out of stock" / "स्टक सकियो" en vez del botón de añadir, y el carrito no admite añadirlo aunque se intente forzar desde la consola.
- **Carrito persistente**: se guarda en `localStorage` en cada cambio, así que si el cliente recarga la página o vuelve más tarde, su carrito sigue ahí.
- **Buscador y filtro por categoría** en tiempo real sobre el catálogo, que busca tanto en el nombre en nepalí como en la traducción al inglés.
- **Carrito de compra** que sobrevive a recargas y cierres de pestaña (guardado en `localStorage`), con contador y total visibles en todo momento.
